...
- Replaced runtime oscillator tones with a complete offline `AudioBuffer` architecture: seven independently configurable/mutable buses, cached decoding, bounded voices, smoothed fades, pause/resume, cleanup generations, autoplay-safe unlocking, spatial pan/depth/occlusion filtering, live-following Husky voices, sparse seeded bird calls, habitat ambience, semantic weapon/score/bird/dog/environment routing, and adaptive menu/briefing/hunt/final/results music.
- Created 42 original procedural audio masters and compact Ogg derivatives without external recordings. Added checksum-backed provenance, `AUDIO_SOURCES_AND_LICENSES.md`, audio asset generation/validation, PWA Ogg precaching, live settings/mute/reduced-ambience controls, browser telemetry, and pure tests for buses, spatial projection, routing, licenses, and offline availability.
...
- Offline spatial-audio validation: `npm run validate:assets`, `npm run check` (60/60 Vitest assertions), and `npm run test:e2e` (15/15 Chromium) passed on 2026-07-22. Browser telemetry verified a running user-unlocked `AudioContext`, live bus/mute changes, distinct shot/miss/reload/empty routing, mapped bird-wing and Husky-bark spatial values, and suspend/resume. All 42 Ogg files appear in the generated service-worker precache; runtime audio totals about 968 KiB and the PWA now precaches 128 entries (about 35.0 MiB). Representative file probes passed and the shot master peaks at −2.1 dBFS. Browser plugin was unavailable, so repository Playwright supplied desktop settings/hunt screenshots and console/page-error checks; the existing non-failing Phaser bundle-size warning remains.
