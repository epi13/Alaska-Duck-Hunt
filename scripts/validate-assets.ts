...
if (
  new Set(birdIds).size !== birdIds.length ||
  new Set(species.map((entry) => entry.id)).size !== species.length
)
  throw new Error('Species and sprite ids must be unique.');
if (birdIds.length !== species.length || species.some((entry) => !birdIds.includes(entry.id)))
  throw new Error(
    'Every content species, target or protected, must have one production sprite definition.',
  );
const locationIds = [
  'matsu',
  'cook',
...
for (const id of birdIds) {
  ...
  if (signature !== 'PNG' || width !== expectedHeight * 4 || height !== expectedHeight) {
    throw new Error(
      `${path} must be a ${expectedHeight * 4}x${expectedHeight} transparent PNG atlas.`,
    );
  }
  ...
  const expectedFrameCount =
    definition.biologicalVariants.length * definition.individualVisualVariants.length * 16;
  if (
    atlas.meta?.speciesId !== id ||
    JSON.stringify(atlas.meta.biologicalVariants) !==
      JSON.stringify(definition.biologicalVariants) ||
    JSON.stringify(atlas.meta.individualVisualVariants) !==
      JSON.stringify(definition.individualVisualVariants) ||
    ...
    throw new Error(
      `${id} atlas must stay within the two-biological-by-two-individual treatment budget (${expectedFrameCount} named frames).`,
    );
  }
  for (const [name, value] of Object.entries(atlas.frames ?? {})) {
    const frame = value.frame;
    if (
      !/^[-a-z]+\/(?:natural|alternate)\/[-A-Za-z]+\/\d+$/.test(name) ||
      frame.x < 0 ||
      frame.y < 0 ||
      frame.w <= 0 ||
      frame.h <= 0 ||
      ...
      throw new Error(`${id} has an invalid or out-of-bounds atlas frame: ${name}.`);
  }
    ...
    for (const individualVisualVariant of definition.individualVisualVariants)
      for (const state of requiredStates) {
        ...
        if (!atlas.frames?.[name])
          throw new Error(`${id} is missing required individual variant frame ${name}.`);
      }
  const preview = pngSize(await readFile(`public/assets/birds/${id}/preview.png`));
  const frameSize = expectedHeight / 4;
  if (preview.width !== frameSize * 4 || preview.height !== frameSize)
    throw new Error(`${id} preview strip is invalid.`);

  if (
    definition.imagePath !== `assets/birds/${id}/atlas.png` ||
    definition.atlasPath !== `assets/birds/${id}/atlas.json`
  )
    throw new Error(`${id} runtime paths do not match the production atlas.`);
  for (const [state, visual] of Object.entries(definition.visuals))
    if (
      !visual ||
      visual.hitbox[0] <= 0 ||
      visual.hitbox[1] <= 0 ||
      ...
      throw new Error(`${id}/${state} has invalid state-specific visual metadata.`);

  const behavior = birdBehaviorBySpecies.get(id);
  if (!behavior) throw new Error(`${id} has no behavior profile.`);
  const compatibleStarts = behavior.surfaces
    .flatMap((surface) =>
      behavior.initialStates.map((state) => ({
        surface,
        state,
  ...
  if (
    waterFamily &&
    !behavior.surfaces.some((surface) =>
      [
        'openWater',
        'shallowWater',
        'mudflat',
    ...
    throw new Error(`${id} lacks an appropriate water/shoreline start surface.`);
}

const protectedSpecies = species.filter((entry) => !entry.target);
if (
  protectedSpecies.length !== 1 ||
  protectedSpecies[0]?.id !== 'spectacled' ||
  birdScoringBySpecies.get('spectacled')?.role !== 'protected'
)
  throw new Error('Spectacled Eider must be the illustrated protected role.');
const craneStanding = scoreBird({
  speciesId: 'crane',
  state: 'standingBonus',
  distance: 0.5,
  speedRatio: 1,
...
if (craneStanding.points <= craneAirborne.points || !craneStanding.label)
  throw new Error('Crane upright scoring must exceed airborne scoring and include a bonus label.');
const craneVisual = birdSprites.find((entry) => entry.speciesId === 'crane')!;
if (
  JSON.stringify(craneVisual.visuals.standingBonus?.hitbox) ===
  JSON.stringify(craneVisual.visuals.flying?.hitbox)
)
  throw new Error('Crane upright and airborne hitboxes must differ.');

for (const rootEntry of await readdir('public/assets/birds', { withFileTypes: true }))
  if (rootEntry.isFile())
    throw new Error(`Obsolete flat bird sheet remains in production: ${rootEntry.name}.`);
const spriteSource = await readFile('src/data/bird-sprites.ts', 'utf8');
if (/assets\/birds\/[a-z-]+\.png/.test(spriteSource))
  throw new Error('Runtime still references an obsolete flat bird sheet.');
const viteConfig = await readFile('vite.config.ts', 'utf8');
if (!viteConfig.includes('png,svg,json,webp,ogg'))
  throw new Error(
    'PWA precache glob must include production WebP UI art, atlas PNG, JSON metadata, and offline Ogg audio.',
  );

const startBackground = await readFile('public/assets/ui/start-copper-river.webp');
if (
  startBackground.subarray(0, 4).toString('ascii') !== 'RIFF' ||
  startBackground.subarray(8, 12).toString('ascii') !== 'WEBP'
) {
  throw new Error('The packaged start-screen background must be a valid WebP.');
}
for (const path of [
  'assets/generated/ui/start-screen/masters/copper-river-master.png',
  'assets/generated/ui/start-screen/concept.png',
  'assets/generated/ui/start-screen/DESIGN_BRIEF_AND_PROMPT.md',
  'assets/generated/ui/start-screen/palette.json',
  'assets/generated/ui/start-screen/SOURCES_AND_LICENSES.md',
])
  await access(path);
const startMasterSize = pngSize(
  await readFile('assets/generated/ui/start-screen/masters/copper-river-master.png'),
);
if (startMasterSize.width !== 1280 || startMasterSize.height !== 720) {
  throw new Error('The lossless start-screen source master must remain 1280x720.');
}
for (const sourcePath of ['src/main.ts', 'src/styles/main.css']) {
  if (/\/docs\/images\//.test(await readFile(sourcePath, 'utf8'))) {
    throw new Error(`${sourcePath} must not depend on docs-only imagery at runtime.`);
  }
}

const audioManifest = JSON.parse(
  await readFile('assets/generated/audio/manifest.json', 'utf8'),
) as {
  generatedAt?: string;
  assets?: Array<{
    id?: string;
...
if (
  audioManifest.generatedAt !== '2026-07-22' ||
  audioManifest.assets?.length !== audioAssets.length
) {
...
for (const definition of audioAssets) {
  ...
  if (
    master.subarray(0, 4).toString('ascii') !== 'RIFF' ||
    processed.subarray(0, 4).toString('ascii') !== 'OggS'
  ) {
    throw new Error(`${definition.id} must retain a WAV master and ship an Ogg derivative.`);
  }
  if (
    createHash('sha256').update(master).digest('hex') !== entry.masterSha256 ||
...
const shippedAudioFiles = (await readdir('public/assets/audio')).filter((name) =>
  name.endsWith('.ogg'),
);
if (
  shippedAudioFiles.length !== declaredAudioFiles.size ||
  shippedAudioFiles.some((name) => !declaredAudioFiles.has(name))
) {
...
if (sceneMaps.map(({ locationId }) => locationId).join(',') !== locationIds.join(','))
  throw new Error('Scene-map catalog must cover all locations in canonical order.');
...
if (scenePropLayouts.map(({ locationId }) => locationId).join(',') !== locationIds.join(','))
  throw new Error('Scene-prop catalog must cover all locations in canonical order.');
for (const map of sceneMaps) {
  ...
  if (invalid.length)
    throw new Error(
      `Invalid ${map.locationId} prop placements: ${invalid.map(({ placementId, errors }) => `${placementId}: ${errors.join(', ')}`).join('; ')}`,
    );
}
...
if (
  huskyAtlas.meta?.characterId !== huskySprite.characterId ||
...
if (huskyPreview.width !== 512 || huskyPreview.height !== 128)
  throw new Error('The Alaskan Husky preview must be 512x128.');
for (const path of [
  ...
  await access(path);
const huskyValidation = JSON.parse(
  await readFile('assets/generated/characters/alaska-husky/validation.json', 'utf8'),
) as {
  characterId?: string;
  checks?: {
    alphaValues?: number[];
    maximumColorsPerFrameIncludingTransparency?: number;
    normalizedPawBaselineY?: number;
    proprietarySourceArtworkUsed?: boolean;
  };
  sha256?: Record<string, string>;
};
if (
  huskyValidation.characterId !== huskySprite.characterId ||
...
for (const [relativePath, expectedHash] of Object.entries(huskyValidation.sha256 ?? {})) {
  const path = relativePath.startsWith('public/')
    ? relativePath
    : `assets/generated/characters/alaska-husky/${relativePath}`;
  const actualHash = createHash('sha256')
    .update(await readFile(path))
    .digest('hex');
  if (actualHash !== expectedHash)
    throw new Error(
      `Alaskan Husky asset checksum changed without updating its validation record: ${path}`,
    );
}
...
console.log(
  `Required original assets are present: ${birdIds.length} named bird atlases, ${locationIds.length} scene plates, the named Alaskan Husky atlas/source/reference set, three legacy habitat atlases, and ${regionalHabitatAtlases.length} regional habitat atlases.`,
);
