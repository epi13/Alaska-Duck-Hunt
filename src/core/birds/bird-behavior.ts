...
export function flightVector(plan: BirdPlan, elapsedMs: number): FlightVector {
  const t = Math.max(0, elapsedMs + plan.glideTimingOffsetMs) / 1_000;
  const direction = plan.direction;
  const baseX = direction * (plan.speed + plan.acceleration * t);
  const localPath = Math.sin(t * 1.35 + plan.movementPhase) * plan.localPathOffset;
  switch (plan.flightProfile) {
    case 'dartingFlight':
      return {
        x: baseX,
        y: -plan.climbRate * 0.35 + Math.sin(t * 5.8 + plan.movementPhase) * 95 + localPath,
        rotation: Math.sin(t * 5.8 + plan.movementPhase) * 0.2,
      };
    case 'lowCoastalFlight':
      return { x: baseX, y: Math.sin(t * 2.1 + plan.movementPhase) * 18 + localPath, rotation: 0 };
    case 'heavyMarineFlight':
      return { x: baseX * 0.88, y: -plan.climbRate * 0.18 + localPath, rotation: -direction * 0.025 };
    case 'gooseFormationFlight':
      return { x: baseX * 0.84, y: -plan.climbRate * 0.28 + Math.sin(t + plan.movementPhase) * 12 + localPath, rotation: 0 };
    case 'shortFlushFlight':
      return { x: baseX * 0.78, y: elapsedMs < 850 ? -plan.climbRate : plan.climbRate * 0.12, rotation: -direction * 0.06 };
    case 'craneFlight':
      return { x: baseX * 0.62, y: -plan.climbRate * 0.22 + Math.sin(t * 0.8 + plan.movementPhase) * 16 + localPath, rotation: 0 };
    case 'circlingReturn':
      return { x: baseX * Math.cos(t * 0.7), y: Math.sin(t * 1.4 + plan.movementPhase) * 48 + localPath, rotation: Math.sin(t * 0.7) * 0.18 };
    case 'localRelocation':
      return { x: baseX * 0.55, y: elapsedMs < 600 ? -plan.climbRate * 0.85 : plan.climbRate * 0.28, rotation: 0 };
    default:
      return { x: baseX, y: -plan.climbRate * 0.16 + Math.sin(t * 1.6 + plan.movementPhase) * 16 + localPath, rotation: 0 };
  }
}
